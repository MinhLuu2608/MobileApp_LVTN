...
import 'package:MobileApp_LVTN/screens/employee/invoice_info.dart';
...
class InvoicesList extends StatefulWidget{
  const InvoicesList({Key? key, required this.filterType}) : super(key: key);
...
class InvoicesListState extends State<InvoicesList> {
  ...
  Future<List<EmpInvoice>> getHoaDon() async {
    box1 = await Hive.openBox('logindata');
    final int IDAccount = box1.get("IDAccount");
    final url = Uri.http(urlAPI, 'api/MobileApp/getEmpHoaDon/$IDAccount/${widget.filterType}');
                          ...
                          children: <Widget>[
                            Column(
                              ...
                              children: <Widget>[
                                ...
                                Padding(
                                  padding: const EdgeInsets.all(5.0),
                                  child: Text("Tuyến thu: ${snapshot.data[index].tenTuyenThu}"),
                                ), //Tên tuyến thu
                              ],
                            ),
                            Expanded(
